...
export default function LoginPage() {
  ...
  const modalEmailRef = useRef<HTMLInputElement>(null) as React.RefObject<HTMLInputElement>;
  ...
  // Focus modal email input when desktop login opens
  useEffect(() => {
    if (showDesktopLogin) {
      setTimeout(() => modalEmailRef.current?.focus(), 100);
    }
  }, [showDesktopLogin]);
  ...
  useEffect(() => {
    ...
    function onKey(e: KeyboardEvent) {
      if (e.key === 'Escape') setShowDesktopLogin(false);
    }
  ...
  /* ── Shared login form fields (used in mobile card and desktop modal) ── */
  function renderLoginFields(inputRef?: React.RefObject<HTMLInputElement>) {
    return (
      <>
        {/* Logo */}
  ...
  return (
    <>
      ...
      {/* ═══════════════════════════════════════════════════════════════════
           MOBILE EXPERIENCE (< lg): Login card + landing sections
           ═══════════════════════════════════════════════════════════════════ */}
      <div
      ...
      >
        ...
        >
          {/* Motivational text (visible on md tablets) */}
      ...
      {/* ═══════════════════════════════════════════════════════════════════
           DESKTOP EXPERIENCE (lg+): Marketing landing page
           ═══════════════════════════════════════════════════════════════════ */}
      <div className="hidden lg:block">
        <DesktopLanding onLoginClick={() => setShowDesktopLogin(true)} />
      </div>
      ...
      {showDesktopLogin && (
        <div
          className="fixed inset-0 z-[100] hidden items-center justify-center lg:flex"
        ...
        >
          ...
          {/* Modal card */}
          <div
            className="relative w-full max-w-[440px] rounded-3xl"
            style={{
              background: 'var(--bb-depth-3)',
              border: '1px solid var(--bb-glass-border)',
              padding: 'clamp(1.5rem, 4vw, 2.75rem) clamp(1.25rem, 3.5vw, 2.25rem)',
          ...
          >
            ...
            <button
              ...
              className="absolute right-4 top-4 flex h-8 w-8 items-center justify-center rounded-lg border-none transition-colors duration-200"
            ...
            {renderLoginFields(modalEmailRef)}
          </div>
        </div>
      )}
    </>
