...
const CookieBanner = forwardRef<HTMLDivElement, { className?: string }>(
  function CookieBanner({ className }, ref) {
    ...
    return (
      <div
        ref={ref}
        className={`fixed bottom-0 left-0 right-0 z-50 border-t border-bb-gray-200 p-4 shadow-lg sm:flex sm:items-center sm:justify-between sm:gap-4 sm:px-6 ${className ?? ''}`}
