...
export function SeriesPlayer({ backHref, accentColor = 'red' }: SeriesPlayerProps) {
  ...
  const accent = accentColor === 'purple' ? 'purple' : accentColor === 'yellow' ? 'yellow' : 'red';
  const accentBg = accent === 'purple' ? 'bg-purple-500' : accent === 'yellow' ? 'bg-yellow-500' : 'bg-red-500';
  const accentBgHover = accent === 'purple' ? 'hover:bg-purple-600' : accent === 'yellow' ? 'hover:bg-yellow-600' : 'hover:bg-red-600';
  const accentBgLight = accent === 'purple' ? 'bg-purple-500/10' : accent === 'yellow' ? 'bg-yellow-500/10' : 'bg-red-500/10';
  const accentBorder = accent === 'purple' ? 'border-purple-500/50' : accent === 'yellow' ? 'border-yellow-500/50' : 'border-red-500/50';
  const accentBorderSelected = accent === 'purple' ? 'border-purple-500' : accent === 'yellow' ? 'border-yellow-500' : 'border-red-500';
  const accentText = accent === 'purple' ? 'text-purple-400' : accent === 'yellow' ? 'text-yellow-400' : 'text-red-400';
  const accentBarBg = accent === 'purple' ? 'bg-purple-500' : accent === 'yellow' ? 'bg-yellow-500' : 'bg-red-500';
  ...
  function isEpisodeCompletedFn(videoId: string): boolean {
    ...
    >
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4 pt-12">
          ...
          >
            <div
              className={`h-full ${accentBarBg} rounded-full relative transition-all`}
              style={{ width: `${progressPercent}%` }}
            >
              <div className={`absolute right-0 top-1/2 -translate-y-1/2 w-4 h-4 ${accentBarBg} rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-lg`} />
            </div>
          </div>
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <button
                onClick={() => setIsPlaying((prev) => !prev)}
                className={`text-white hover:${accentText} transition-colors`}
              >
                {isPlaying ? (
        ...
        <div className="flex-1 p-5 lg:w-[60%]">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-4">
            ...
            {detail.series.modality && (
              <span className={`text-xs ${accentBgLight} ${accentText} px-2 py-1 rounded-full`}>
                {detail.series.modality}
              </span>
            )}
          ...
          <div className="lg:hidden">
            ...
            <div className="flex flex-col gap-2">
              {detail.series.videos.map((video, idx) => {
                ...
                const completed = isEpisodeCompletedFn(video.id);
                return (
                  <button
                    key={video.id}
                  ...
                  {currentQuestions.map((_, i) => (
                    <div
                      ...
                      className={`h-1 flex-1 rounded-full transition-all ${
                        i <= quizStep ? accentBarBg : 'bg-white/10'
                      }`}
                    />
                  ))}
                </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-2">
                      {currentQuestion.options.map((option, optIdx) => {
                        ...
                        const isSelected = selectedAnswers[quizStep] === optIdx;
                        return (
                          <button
                            key={optIdx}
                            ...
                            className={`flex items-center gap-3 p-4 rounded-xl border text-left transition-all ${
                              isSelected
                                ? `${accentBorderSelected} ${accentBgLight} text-white`
                    ...
                    <Button
                      ...
                      className={`w-full mt-6 ${accentBg} ${accentBgHover} disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold py-3 rounded-xl transition-all`}
                    >
                      {quizStep < currentQuestions.length - 1 ? 'Próxima' : 'Confirmar'}
                    </Button>
                  </div>
                )}
              </>
                ...
                <div className="text-center py-4">
                  <div className="text-5xl mb-4">
                    {quizResult.score === currentQuestions.length
                      ? '🏆'
                      : quizResult.score >= currentQuestions.length / 2
                        ? '👏'
                        : '📖'}
                  </div>

                  <h2 className="text-xl font-bold text-white mb-2">
                    Resultado: {quizResult.score}/{currentQuestions.length}
                  </h2>

                  {quizResult.score === currentQuestions.length ? (
                    <p className="text-green-400 font-medium mb-2">Parabéns! Acertou tudo!</p>
                  ) : (
                    <p className="text-gray-400 text-sm mb-2">Continue estudando para melhorar!</p>
                  )}

                  <div className="inline-flex items-center gap-2 bg-yellow-500/10 border border-yellow-500/30 rounded-full px-4 py-2 mb-6">
                    <span className="text-yellow-400 font-bold">+{quizResult.xp_gained} XP</span>
                  </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <p className="text-sm text-white font-medium">Pergunta {i + 1}</p>
                            {!isCorrect && wrong?.hint && (
                              <p className="text-xs text-red-400 mt-1">{wrong.hint}</p>
                            )}
                            {isCorrect && (
                              <p className="text-xs text-green-400 mt-1">+10 XP</p>
                            )}
                          </div>
                        </div>
                      );
                  ...
                  <Button
                    ...
                    className={`w-full ${accentBg} ${accentBgHover} text-white font-bold py-3 rounded-xl transition-all`}
                  >
                    {episodeIndex < detail.series.videos.length - 1 ? 'Próximo episódio' : 'Finalizar'}
                  </Button>
                </div>
              </>
            )}
        ...
        <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#14141a] border border-yellow-500/30 rounded-2xl w-full max-w-md p-8 text-center">
            ...
            <div className="flex justify-center gap-6 mb-6">
              <div className="text-center">
                <p className="text-2xl font-bold text-white">{detail.series.videos.length}</p>
                <p className="text-xs text-gray-500">Episódios</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold text-white">
                  {formatTime(detail.series.videos.reduce((acc, v) => acc + v.duration_seconds, 0))}
                </p>
            ...
            <div className="flex flex-col gap-3">
              <Button
                onClick={() => toast('Certificado será enviado para seu e-mail!', 'success')}
