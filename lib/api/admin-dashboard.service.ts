...
export async function getDashboardData(academyId: string): Promise<DashboardData> {
  try {
    ...
    console.warn('[getDashboardData] Fallback:', error);
    return {
      greeting: { admin_name: 'Admin', academy_name: '', time_of_day: 'morning', motivation_quote: '' },
      headlines: { active_students: { value: 0, change: 0, trend: 'stable', period: 'month' }, monthly_revenue: { value: 0, change: 0, trend: 'stable', period: 'month' }, retention_rate: { value: 0, change: 0, trend: 'stable', period: 'month' }, classes_this_week: { value: 0, total_capacity: 0, fill_rate: 0 } },
      growth_chart: { labels: [], students: [], revenue: [], churn: [] },
      retention: { current_month: 0, previous_month: 0, at_risk: 0, at_risk_names: [] },
      today_schedule: [], activity_feed: [], pending_promotions: [],
      financial_summary: { revenue_this_month: 0, revenue_last_month: 0, pending_payments: 0, overdue_count: 0, overdue_names: [] },
      plan_usage: { plan_name: '', students: { current: 0, limit: 0 }, professors: { current: 0, limit: 0 }, classes: { current: 0, limit: 0 }, storage_gb: { current: 0, limit: 0 }, alerts: [] },
      streaming_summary: { total_views_week: 0, most_watched: { title: '', views: 0 }, completion_rate: 0, new_videos_this_month: 0 },
      quick_actions: [], academy_achievements: [],
    };
  }
}
...
export async function getActivityFeed(
  ...
  try {
    ...
    if (error) {
      console.warn('[getActivityFeed] Supabase error:', error.message);
      return [];
    }
    ...
    console.warn('[getActivityFeed] Fallback:', error);
    return [];
  }
}
