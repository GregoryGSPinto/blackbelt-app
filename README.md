...
- `app.blackbelt.com`: app autenticado, auth, onboarding operacional e superfícies públicas do produto
- `api.blackbelt.com`: rotas `app/api`, webhooks e contratos HTTP do produto
...
O site comercial vive em `blackbelt.com` e está representado aqui apenas por `scaffolds/blackbelt-site` e por rotas de redirecionamento em `app/(site-marketing)`.
...
- `app/(site-marketing)`: rotas legadas que redirecionam para `blackbelt.com`
...
- mudanças de domínio e topologia devem manter o contrato `blackbelt.com` vs `app.blackbelt.com` vs `api.blackbelt.com`
